...
def visualise(q, shape):
    ...
    log_qzk = tf.math.reduce_sum(log_qz0, axis=1, keepdims=True) - log_det_jacobian
    qzk = tf.math.exp(log_qzk).numpy().flatten()

    zk = zk.numpy()

    # Reorder z based on probability value to make cmap look good
    q_indx_sort = qzk.argsort()
    zk = zk[q_indx_sort]
    qzk = qzk[q_indx_sort]

    thetas, mus, log_tau = zk[:, 0:eight_schools_K], zk[:, -2], zk[:, -1]
    ...
    plt.scatter(log_tau, thetas, c=qzk, alpha=0.8, cmap='magma', label='ADVI+NF Samples')
    ...
    sns.kdeplot(thetas, color='darkmagenta',
                ax=ax1, label=rf'kde($\theta_{i + 1}$' + r'$)_{ADVI+NF}$')
    ax1.legend()
    ...
    sns.kdeplot(mus, color='darkmagenta',
                ax=ax2, label=r'kde($\mu)_{ADVI+NF}$')
    ...
    sns.kdeplot(tf.math.exp(log_tau).numpy(), color='darkmagenta',
                ax=ax3, label=r'kde($\tau)_{ADVI+NF}$')
